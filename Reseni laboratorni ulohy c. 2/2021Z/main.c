...
/*                                                                            */
/*  Reseni vytvoril(a) a odevzdava: (Slavomir Svorada, xsvora02)                            */
/*                                                                            */
/******************************************************************************/
...
void PORTB_IRQHandler(void) {
	...
	} else if (PORTB->ISFR & BUTTON_DOWN_MASK) {
		if (!(GPIOB->PDIR & BUTTON_DOWN_MASK)) {
			beep();
			PORTB->ISFR |= BUTTON_DOWN_MASK;
		}
	} else if (PORTB->ISFR & BUTTON_CENT_MASK) {
		if (!(GPIOB->PDIR & BUTTON_CENT_MASK)) {
			flash();
			beep();
